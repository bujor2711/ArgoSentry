...
InputState::InputState(const DMA& dma) : dma(dma) {
    ...
    if (!VMMDLL_ConfigGet(dma.get_handle().get(), VMMDLL_OPT_WIN_VERSION_BUILD, &windows_version_build)) {
...
bool InputState::retrieve_gafAsyncKeyState(const std::vector<DWORD>& csrss_process_ids) {
    ...
    if (windows_version_build > 22000) {
        ...
        for (const DWORD& process_id : csrss_process_ids) {
            ...
            if (VMMDLL_Map_GetModuleFromNameW(dma.get_handle().get(), process_id, const_cast<LPWSTR>(L"win32ksgd.sys"), win32k_module_info.out(), VMMDLL_MODULE_FLAG_NORMAL)) {
                win32k_module_name = "win32ksgd.sys";
            }
            else if (VMMDLL_Map_GetModuleFromNameW(dma.get_handle().get(), process_id, const_cast<LPWSTR>(L"win32k.sys"), win32k_module_info.out(), VMMDLL_MODULE_FLAG_NORMAL)) {
            ...
            if (!VMMDLL_Map_GetModuleFromNameW(dma.get_handle().get(), process_id, const_cast<LPWSTR>(L"win32kbase.sys"), win32kbase_info.out(), VMMDLL_MODULE_FLAG_NORMAL)) {
    ...
    if (!VMMDLL_Map_GetEATU(dma.get_handle().get(), winlogon_process_id | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, const_cast<LPSTR>("win32kbase.sys"), eat_map.out()) || eat_map->dwVersion != VMMDLL_MAP_EAT_VERSION) {
...
bool InputState::retrieve_gptCursorAsync(const std::vector<DWORD>& csrss_process_ids) {
    ...
    for (const DWORD& process_id : csrss_process_ids) {
        VolkResource<VMMDLL_MAP_EAT> eat_map;
        if (!VMMDLL_Map_GetEATU(dma.get_handle().get(), process_id | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, const_cast<LPSTR>("win32kbase.sys"), eat_map.out())) {
...
bool InputState::read_bitmap() {
    ...
    return VMMDLL_MemReadEx(dma.get_handle().get(), winlogon_process_id | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, gafAsyncKeyState_address, reinterpret_cast<PBYTE>(&state_bitmap), sizeof(state_bitmap), nullptr, VMMDLL_FLAG_NOCACHE);
}
